        ...
        for p in todos:
            ...
            if fecha_obj == hoy:
                ...
                partidos_hoy.append(
                    ...
                    f"Probabilidades: Local {p['ProbLocal']:.0f}% (Cuota: {cuota(p['ProbLocal'])}), "
                    f"Empate {p['ProbEmpate']:.0f}% (Cuota: {cuota(p['ProbEmpate'])}), "
                    f"Visita {p['ProbVisita']:.0f}% (Cuota: {cuota(p['ProbVisita'])})\n"
                    f"Doble oportunidad: 1X {p.get('Prob1X', 0):.0f}% (Cuota: {cuota(p.get('Prob1X', 0))}), "
                    f"12 {p.get('Prob12', 0):.0f}% (Cuota: {cuota(p.get('Prob12', 0))}), "
                    f"X2 {p.get('ProbX2', 0):.0f}% (Cuota: {cuota(p.get('ProbX2', 0))})\n"
                    f"Over 0.5: {p.get('ProbOver05', 0):.0f}% (Cuota: {cuota(p.get('ProbOver05', 0))}) | "
                    f"Over 1.5: {p.get('ProbOver15', 0):.0f}% (Cuota: {cuota(p.get('ProbOver15', 0))}) | "
                    f"Over 2.5: {p.get('ProbOver25', 0):.0f}% (Cuota: {cuota(p.get('ProbOver25', 0))})\n"
                    ...
                    f"Ambos marcan: Sí {p.get('ProbAmbosMarcan', 0):.0f}% (Cuota: {cuota(p.get('ProbAmbosMarcan', 0))}), "
                    f"No {p.get('ProbNoAmbosMarcan', 0):.0f}% (Cuota: {cuota(p.get('ProbNoAmbosMarcan', 0))})\n"
                    f"Estado: {estado}\n"
                    "-----------------------------"
                )
